...
{
    private const string k_googleSheetUrl = "1MnwQSOcPjcSvc-CUu81X9oZgmo7TJCtTrGgaXljHt6s";
    private const string k_guidType = "&gid=";
    private const string url = "https://docs.google.com/spreadsheets/d/";
    private const string exportType =  "/export?format=csv";

    internal static IEnumerator DownloadData(System.Action<string> onCompleted, string googleSheetUrl, string guidId)
    {
        yield return new WaitForEndOfFrame();

        string fullUrl = url + googleSheetUrl + exportType + k_guidType + guidId;
